...
public:
    int maxValue(vector<vector<int>> &events, int k)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            int last = upper_bound(endTimes.begin(), endTimes.end(), start - 1) - endTimes.begin();
            for (int j = 1; j <= k; j++)
            {
                dp[i][j] = max(dp[i][j], dp[i - 1][j]);
                dp[i][j] = max(dp[i][j], dp[last][j - 1] + val);
            }
        }
        return dp[n][k];
    }
...
int main()
{
    ...
    cout << "Maximum value by attending up to " << k << " events: " << result << endl;
